<template>
  <div :class="{'rtl': rtl}">
    <div>
      <div v-if="title" class="paginationclass">
        <h2 class="table-title pull-left">{{title}}</h2>
        <div class="actions pull-right">
        </div>
      </div>
  
      <!-- <vue-good-pagination v-if="paginate && paginateOnTop" :perPage="perPage" :rtl="rtl" :total="processedRows.length" @page-changed="pageChanged" @per-page-changed="perPageChanged"></vue-good-pagination> -->
  
      <table class="table table-striped">
        <!-- <table ref="table" :class="styleClass"> -->
        <thead>
          <tr class="paginationclass" v-if="globalSearch && externalSearchQuery == null">
          ...
          <tr class="paginationclass">
            ...
            <th v-for="(column, index) in columns" :key="column.field" @click="sort(index)" :class="getHeaderClasses(column, index)" :style="{width: column.width ? column.width : 'auto'}" v-if="!column.hidden">
              <slot name="table-column" :column="column">
                <span>{{column.label}}</span>
              </slot>
            </th>
            <!-- ##########   title with create new  -->
            <th v-if="globalSearch && externalSearchQuery !== null" :style="{colspan:2}" >
              <button @click="createClick()" class="btn btn-sm btn-info pull-right">
      									<span class="glyphicon glyphicon-plus"></span> 추가
      								</button>
            </th>
            <slot name="thead-tr"></slot>
          </tr>
          <tr v-if="hasFilterRow" class="paginationclass">
            ...
            <th v-for="(column, index) in columns" :key="column.field" v-if="!column.hidden">
              <div v-if="column.filterable" :class="getHeaderClasses(column, index)">
                <input v-if="!column.filterDropdown" type="text" class="" 
                :value="columnFilters[column.field]" v-on:input="updateFilters(column, $event.target.value)" />
  
                <!-- options are a list of primitives -->
                <select v-if="column.filterDropdown && typeof(column.filterOptions[0]) !== 'object'" class="" :value="columnFilters[column.field]" v-on:input="updateFilters(column, $event.target.value)">
        ...
        <tbody>
          <tr v-for="(row, index) in paginated" :key="index" :class="getRowStyleClass(row)" @click="click(row, index)">
            ...
            <slot name="table-row" :row="row" :formattedRow="formattedRow(row)" :index="index">
              <td v-for="(column, i) in columns" :key="column.field" :class="getClasses(i, 'td')" v-if="!column.hidden && column.field">
                <span v-if="!column.html">{{ collectFormatted(row, column) }}</span>
                <span v-if="column.html" v-html="collect(row, column.field)"></span>
              </td>
              <!-- #####   edit & delete button ###### -->
              <td>
                <div v-if="globalSearch && externalSearchQuery !== null" class="form-inline pull-right">
                  <button type="button" @click="editClick(row , index)"  class="btn btn-sm btn-info">
                        <span class="glyphicon glyphicon-edit"></span> 상세정보
                      </button>
                  <button type="button" @click="deleteClick(row , index)" class="btn btn-sm btn-warning">
      ...
      <div align="center">
         <b-pagination :total-rows="processedRows.length" :per-page="10" v-model="currentPage" class="my-0" /> 
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editClick(row, index) {
        if (this.onEditClick)
          this.onEditClick(row, index);
      },
      deleteClick(row, index) {
        if (this.onDeleteClick)
          this.onDeleteClick(row, index);
      },
      searchTable() {
      ...
      getClasses(index, element) {
        ...
        let isRight = [ 'percentage', 'decimal'].includes(type);
